...
class AssetsListVC: UIViewController {
    ...
    private func setButtonEvents() {
        assetsListView.saveButton.addTarget(self, action: #selector(saveButtonTapped), for: .touchUpInside)
        assetsListView.cancelButton.addTarget(self, action: #selector(cancelButtonTapped), for: .touchUpInside)
    }
    
    private func setDeleteButton() {
        assetsListView.deleteButton.target = self
        assetsListView.deleteButton.action = #selector(deleteButtonTapped)
        navigationItem.rightBarButtonItem = assetsListView.deleteButton
    }
